...
export class VoucherComponent implements OnInit, OnDestroy {
    ...
    if (this.editMode) {
      if (this.voucherId > 0) {
        ...
        this.swServ.showErrorMessage(
          "Error!!!",
          "Unable to get voucher details with given input. Please try again"
        );
      }
    } else {
  ...
  initForm() {
    if (this.editMode) {
      if (this.isLe && this.stationId > 0) {
        ...
        if (
          vouch.taxAmount > 0 &&
          vouch.taxAmount != null &&
          vouch.taxAmount != NaN &&
          vouch.taxAmount != undefined
        ) {
          etax = "gst";
        }
        ...
        this.swServ.showErrorMessage(
          "Error!!",
          "Unable to get input details to edit voucher process, Please try again"
        );
      ...
      if (this.isLe) {
        if (this.stationId > 0) {
          ...
          this.swServ.showErrorMessage(
            "Error!!",
            "Unable to get input details to create voucher process, Please try again"
          );
        }
      } else {
        this.voucherForm = this._fb.group({
    ...
    if (
      ...
      if (
        ...
        if (this.editMode) {
          if (
            vouch.VoucherId == 0 ||
            vouch.VoucherId == null ||
            vouch.VoucherId == NaN ||
            vouch.VoucherId == undefined
          ) {
            this.swServ.showErrorMessage(
              "Error!!",
              "Unable to get details to edit the voucher"
            );
          } else {
            this.api.updateVoucher(vouch, this.tkn).subscribe(
              (data: APIResult) => {
                //console.log(data);
                let status: Boolean = data.status;
                let m: string = data.message;
          ...
          this.api.createVoucher(vouch, this.tkn).subscribe(
            (data: APIResult) => {
              //console.log(data);
              let status: Boolean = data.status;
              let m: string = data.message;
  ...
  showrequiredMessage(field, txt, title) {
    ...
    if (txt == "" || txt == null) {
      ...
      field == "Voucher NetAmount" ||
      field == "Voucher TotalAmount" ||
      field == "Voucher TaxAmount"
    ) {
