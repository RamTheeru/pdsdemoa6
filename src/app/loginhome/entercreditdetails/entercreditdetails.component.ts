...
export class EntercreditdetailsComponent implements OnInit {
      ...
      this.api.previousmonthCreditDetails(this.stationId, this.tkn).subscribe(
        (data: APIResult) => {
          ...
          if (status) {
            ...
            m =
              m +
              " and amount already credited in this month for this station : (" +
              nc +
              "); Amount debited in this amount : (" +
              nd +
              "); Balance amount remaining for current month : (" +
              nb +
              ");";
      ...
      if (!test) {
        ...
        if (this.checkforUpdate) {
          swal({
            title: "Are you sure?",
            text:
              "the total credit amount you entered again for this station in current month will be added to existing amount you already entered this month and also update the debits in ledger table for this month, ARE YOU SURE?",
            type: "warning",
  ...
  submitoAPI(ledge: Ledger) {
    this.api.insertCredit(ledge, this.tkn).subscribe(
      (data: APIResult) => {
        //console.log(data);
        let status: Boolean = data.status;
        let m: string = data.message;
